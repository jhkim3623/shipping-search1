...
# ══════════════════════════════════════════════════════════
# 안전한 DataFrame 표시 함수
# - Pandas Styler 완전 미사용
# - NaN/inf/None/Period/datetime 안전 처리
# - 금액 천단위 콤마
# - pinned column 지원
# ══════════════════════════════════════════════════════════
def clean_and_safe_display(
    ...
    for col in display_df.columns:
        ...
        if pd.api.types.is_period_dtype(s):
            display_df[col] = s.astype(str).replace({"NaT": ""})
    ...
    pinned_cols = [str(c) for c in (pinned_cols or []) if str(c) in display_df.columns]
    ...
    for col in display_df.columns:
        ...
        if col == "분석_내역":
            column_config[col] = st.column_config.TextColumn(
                col, width="large", pinned=pinned
            )
            continue

        if col in ["거래처", "품목코드", "품목명(공식)", "점착제코드", "점착제명",
                   "월", "최근날짜", "구분", "가로폭이력", "순위"]:
            column_config[col] = st.column_config.TextColumn(
                col,
                width="large" if col in ["품목명(공식)", "가로폭이력"] else "medium",
                pinned=pinned
            )
            continue
            ...
            if not analysis_results:
                ...
                if selected_customer != "선택하세요":
                    ...
                    if "품목코드" in customer_data.columns and not customer_data.empty:
                        ...
                        comparison_data = []
                        for prod_code in customer_pivot_reset["품목코드"]:
                            prod_row = customer_pivot_reset[customer_pivot_reset["품목코드"] == prod_code]
