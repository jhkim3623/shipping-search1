...
# ══════════════════════════════════════════════════════════
# [핵심] 안전한 데이터프레임 표시 함수 (에러 방지 + 천단위 콤마)
# ══════════════════════════════════════════════════════════
def safe_display_dataframe(df, height=None, key=None, editable=False):
    """
    StreamlitAPIException을 방지하면서 천단위 콤마를 적용하는 안전한 함수
    """
...
with tab1:
    ...
    if q.empty:
        ...
        safe_display_dataframe(g.sort_values(sc) if sc else g)
...
with tab2:
    ...
    if q.empty:
        ...
        safe_display_dataframe(g2.sort_values(sc) if sc else g2)
...
with tab3:
    ...
    if q_ref.empty:
        ...
        safe_display_dataframe(overview[ov_show].sort_values(sc) if sc else overview[ov_show])
        ...
        if ("날짜" in q_ref.columns and "금액(원)" in q_ref.columns and "품목코드" in q_ref.columns):
            ...
            with st.expander("📋 월별 수치 상세 보기 (품목별 월간 판매금액)"):
                ...
                edited_pivot = safe_display_dataframe(pivot_reset, key="monthly_detail_editor", editable=True)
...
• 종합평가: {"⚠️ 긴급" if total_score > 70 else "🔍 주의" if total_score > 50 else "📋 모니터링"}"""
                ...
                return {
                    ...
                    "하락률(%)": round(decline_rate * 100, 1) if avg_first > 0 else 0,
            ...
            if not analysis_results:
                ...
                edited_priority = safe_display_dataframe(
                ...
                if selected_customer != "선택하세요":
                    ...
                    if "품목코드" in customer_data.columns and not customer_data.empty:
                        ...
                        edited_customer_pivot = safe_display_dataframe(
                        ...
                        for prod in customer_pivot_reset["품목코드"]:
                            ...
                            if first_cols and last_cols:
                                ...
                                comparison_data.append({
                                    ...
                                    "변화율(%)": round(((last_avg - first_avg) / first_avg * 100), 1) if first_avg > 0 else 0
                                })
                        ...
                        if comparison_data:
                            ...
                            safe_display_dataframe(comp_df)
                    else:
