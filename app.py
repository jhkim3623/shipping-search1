...
# ══════════════════════════════════════════════════════════
# [핵심] 안전한 DataFrame 처리 함수 (StreamlitAPIException 완전 방지)
# ══════════════════════════════════════════════════════════
def clean_and_safe_display(df, height=None, key=None, editable=False):
    """
    StreamlitAPIException을 완벽하게 방지하는 안전한 DataFrame 표시 함수
    - Pandas Styler 사용 안 함
    - 모든 특수값(NaN, inf, None) 안전 처리
    - Streamlit Native Column Config 활용
    """
    # 1. 데이터 안전화 처리
    display_df = df.reset_index(drop=True).copy()
    
    # 2. 특수값 처리 (NaN, inf, None → 안전한 값으로 변환)
    for col in display_df.columns:
        if pd.api.types.is_numeric_dtype(display_df[col]):
            # 무한대를 NaN으로 변환 후 0으로 채움
            display_df[col] = display_df[col].replace([np.inf, -np.inf], np.nan).fillna(0)
        else:
            # 문자열 컬럼의 NaN, None 처리
            display_df[col] = display_df[col].fillna("").astype(str)
            display_df[col] = display_df[col].replace(["nan", "None", "<NA>"], "")
    
    # 3. Streamlit Native Column Config 설정 (Styler 대신)
    column_config = {}
    for col in display_df.columns:
        # 텍스트 컬럼은 기본 설정
        ...
        # 숫자 컬럼 천단위 콤마 설정
        if pd.api.types.is_numeric_dtype(display_df[col]):
            if any(k in col for k in ["단가", "금액", "매출", "원", "감소액", "차액", 
                                     "전반부", "후반부", "전체", "합계", "총", "평균"]):
                ...
                column_config[col] = st.column_config.NumberColumn(col, format="%.1f%%")
            elif any(k in col for k in ["점수", "AI", "우선순위"]):
                column_config[col] = st.column_config.NumberColumn(col, format="%.1f")
            else:
                column_config[col] = st.column_config.NumberColumn(col, format="%,d")
    
    # 4. 안전한 표시 (순수 DataFrame만 전달)
    if editable and key:
...
with tab1:
    ...
    if q.empty:
        ...
        g = (q.groupby(uc, dropna=False)
              ...
              .reset_index())
        g["가중평균단가"] = np.where(
            g["총량_M2"]>0, (g["매출액"]/g["총량_M2"]).round(0), 0)
        sc = [c for c in ["거래처","품목코드"] if c in g.columns]
        clean_and_safe_display(g.sort_values(sc) if sc else g)
...
with tab2:
    ...
    if q.empty:
        ...
        g2 = (q.groupby(uc, dropna=False)
               ...
               .reset_index())
        g2["가중평균단가"] = np.where(
            g2["총량_M2"]>0, (g2["매출액"]/g2["총량_M2"]).round(0), 0)
        sc = [c for c in ["품목코드","거래처"] if c in g2.columns]
        clean_and_safe_display(g2.sort_values(sc) if sc else g2)
...
with tab3:
    ...
    st.caption("단가 0 및 샘플 품목 자동 제외 | 소형 하위35% / 중형 중간40% / 대형 상위25%")
    ...
    if q_ref.empty:
        ...
        clean_and_safe_display(overview[ov_show].sort_values(sc) if sc else overview[ov_show])
        ...
        if ("날짜" in q_ref.columns and "금액(원)" in q_ref.columns and "품목코드" in q_ref.columns):
            ...
            with st.expander("📋 월별 수치 상세 보기 (품목별 월간 판매금액)"):
                ...
                edited_pivot = clean_and_safe_display(pivot_reset, key="monthly_detail_editor", editable=True)
                if edited_pivot is not None:
                    csv_pivot = edited_pivot.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
                    st.download_button("📥 월별 수치 CSV 다운로드", data=csv_pivot, 
                                     file_name="월별_품목별_판매금액.csv", mime="text/csv")

# ══════════════════════════════════════════════════════════
# TAB 4 — 매출 하락 분석 (완전 개선 + AI 분석)
# ══════════════════════════════════════════════════════════
with tab4:
    st.subheader("📉 AI 기반 매출 하락 업체 분석")
    st.caption("🎯 모든 금액 데이터는 천단위 콤마(,)로 표시됩니다")
    
    with st.expander("ℹ️ 전반부/후반부 매출 산출 방식 및 AI 분석 로직"):
        st.markdown("""
### **📊 매출 산출 방식**

**기간 분할 방법:**
- 전체 분석 기간의 월 데이터를 시간 순서대로 정렬
- 중간 지점을 기준으로 전반부와 후반부로 균등 분할

**수학적 공식:**


$$\\text{전반부 평균매출} = \\frac{\\sum_{i=1}^{n/2} \\text{월별매출}_i}{n/2}$$


$$\\text{후반부 평균매출} = \\frac{\\sum_{i=n/2+1}^{n} \\text{월별매출}_i}{n/2}$$


$$\\text{매출 감소액} = \\text{후반부 평균매출} - \\text{전반부 평균매출}$$

### **🤖 AI 우선순위 점수 (100점 만점)**

| 지표 | 배점 | 계산 방식 |
|------|------|-----------|
| **매출 규모** | 30점 | (해당업체 총매출 ÷ 최대업체 총매출) × 30 |
| **하락 심각도** | 25점 | 하락률 × 25 (최대 25점) |
| **품목 다양성 감소** | 20점 | (감소 품목수 ÷ 전반부 품목수) × 20 |
| **변동성 증가** | 15점 | 변동계수(CV) × 15 |
| **최근 추세** | 10점 | 최근 3개월 하락률 × 10 |
    ...
    if q.empty or "날짜" not in q.columns or "금액(원)" not in q.columns or "거래처" not in q.columns:
        ...
        if len(all_months) < 2:
            ...
            def calculate_ai_priority_score(cust, cust_monthly, all_cust_data):
                ...
                # 분석 내역 자동 생성 (한 줄 요약 형식)
                analysis_text = (
                    f"매출규모:{sales_score:.1f}점(총{int(total_sales):,}원) | "
                    f"하락:{decline_score:.1f}점({decline_rate*100:.1f}%) | "
                    f"품목:{diversity_score:.1f}점 | "
                    f"변동:{volatility_score:.1f}점 | "
                    f"추세:{trend_score:.1f}점 | "
                    f"평가:{'긴급' if total_score>70 else '주의' if total_score>50 else '모니터링'}"
                )
            ...
            if not analysis_results:
                ...
                st.caption("💡 AI 종합 점수순 정렬 | 분석 내역 편집 및 CSV 다운로드 가능")
                
                display_cols = ["순위", "거래처", "AI_우선순위점수", "전체_매출액", 
                               "전반부_평균매출", "후반부_평균매출", "매출_감소액", "하락률(%)", "분석_내역"]
                ...
                if edited_priority is not None:
                    ...
                    st.download_button("📥 우선 대응 업체 분석 결과 CSV 다운로드", 
                                     data=csv_priority, file_name="AI분석_우선대응업체.csv", mime="text/csv")
                ...
                selected_customer = st.selectbox(
                    "분석할 업체를 선택하세요",
                    options=["선택하세요"] + [
                        f"{row['거래처']} (점수: {row['AI_우선순위점수']:.1f}점, 감소액: {row['매출_감소액']:,}원)" 
                ...
                if selected_customer != "선택하세요":
                    ...
                    if "품목코드" in customer_data.columns and not customer_data.empty:
                        customer_pivot = customer_data.pivot_table(
                            index="품목코드", columns="월", values="금액(원)", aggfunc="sum", fill_value=0)
                        ...
                        customer_pivot = customer_pivot.sort_values("_decline")  # 하락 큰 순
                        ...
                        # ═══════════════════════════════════════════════════════════
                        # [수직 레이아웃 3] 품목별 판매 추이 그래프 3종
                        # ═══════════════════════════════════════════════════════════
                        st.markdown(f"### 📈 [{selected_cust_name}] 품목별 판매 추이 시각화")
                        st.caption("💡 3가지 그래프로 매출 하락 주도 품목을 파악하세요")
                        ...
                        # 1. 라인 차트 (추세 분석)
                        fig_line = px.line(
                            customer_monthly, x="월", y="금액(원)", color="품목코드",
                            title=f"1️⃣ 품목별 매출 추세 (Line Chart)", 
                            labels={"금액(원)":"매출액(원)", "월":""}, 
                            markers=True
                        )
                        fig_line.update_layout(
                            xaxis_tickangle=-45, height=450, yaxis_tickformat=",",
                        ...
                        fig_area = px.area(
                            customer_monthly, x="월", y="금액(원)", color="품목코드",
                            title=f"2️⃣ 품목별 매출 구성 (Area Chart)",
                            labels={"금액(원)":"매출액(원)", "월":""}
                        )
                        ...
                        if comparison_data:
                            ...
                            fig_bar.add_trace(go.Bar(
                                x=comp_df["품목코드"], y=comp_df["전반부_평균"], 
                                name="전반부 평균", marker_color="#3498db",
                                text=[f"{v:,}" for v in comp_df["전반부_평균"]],
                                textposition="outside"
                            ))
                            fig_bar.add_trace(go.Bar(
                                x=comp_df["품목코드"], y=comp_df["후반부_평균"], 
                                name="후반부 평균", marker_color="#e74c3c",
                                text=[f"{v:,}" for v in comp_df["후반부_평균"]],
                                textposition="outside"
                            ))
                            fig_bar.update_layout(
                                title=f"3️⃣ 전반부 vs 후반부 평균 매출 비교 (Bar Chart)", 
                                barmode="group", height=450, yaxis_tickformat=","
                            )
                            st.plotly_chart(fig_bar, use_container_width=True)
                            
                            st.markdown("##### 📋 품목별 변화율 상세 (천단위 콤마)")
                            clean_and_safe_display(comp_df)
                    else:
