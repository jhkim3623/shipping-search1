...
def load_excel(file_bytes):
    ...
    def normalize(df, col):
        if col in df.columns:
            ...
            df[col] = s.replace({"":pd.NA,"0":pd.NA,"0.0":pd.NA,"nan":pd.NA,"NaN":pd.NA,"None":pd.NA,"<NA>":pd.NA})
...
with tab1:
    ...
    if q.empty: st.warning("조건에 맞는 데이터가 없습니다.")
        ...
        g = (q.groupby(uc, dropna=False)
              ...
              .reset_index())
        g["가중평균단가"] = np.where(g["총량_M2"]>0,(g["매출액"]/g["총량_M2"]).round(0),np.nan)
...
with tab2:
    ...
    if q.empty: st.warning("조건에 맞는 데이터가 없습니다.")
        ...
        g2 = (q.groupby(uc, dropna=False)
               .agg(출고횟수=("수량(M2)","count"),총량_M2=("수량(M2)","sum"),매출액=("금액(원)","sum"))
               .reset_index())
        g2["가중평균단가"] = np.where(g2["총량_M2"]>0,(g2["매출액"]/g2["총량_M2"]).round(0),np.nan)
...
with tab3:
    ...
    if q_ref.empty:
        ...
        if not ct.empty:
            ...
            m2.metric("적용 기간", f"{n_months:,}개월")
            if "총량_M2" in overview.columns:
                m3.metric("총 출고량", f"{overview['총량_M2'].sum():,.1f} M²")
            if "총매출액" in overview.columns:
                m4.metric("총 매출액", f"{overview['총매출액'].sum():,.0f} 원")
