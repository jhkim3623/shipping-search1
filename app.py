...
def safe_make_product_label(df):
    ...
    if "재단구분" not in temp.columns: temp["재단구분"] = ""
    if "기준폭" not in temp.columns: temp["기준폭"] = ""
...
def clean_and_safe_display(
    ...
    fixed_text_like_cols = {
        ...
        "최근날짜", "재단구분", "기준폭", "가로폭이력", "분석_내역", "AI분석", "업체성향", "최근추세",
        "월", "구분", "월표기", "연도", "대표구분", "반품원인", "주요반품원인",
        "원인추정", "감소원인", "증가원인", "비고", "AI반품분석", "재단구분", "기준폭", "담당부서", "영업담당부서", "담당자",
        "진행현황", "최근진행현황", "분석대상"
    }
    ...
    for col in display_df.columns:
        ...
        if col in text_cols or col in fixed_text_like_cols:
            ...
            if col in ["거래처", "품목코드", "점착제코드", "점착제명", "최근날짜", "최근일자", "진행현황", "분석대상"]:
                width_value = "small"
            if col in [
                "품목명(공식)", "품목표시", "재단구분", "기준폭", "재단구분", "기준폭", "가로폭이력", "분석_내역",
...
def load_excel(file_bytes):
    ...
    for c in ["거래처", "품목코드", "점착제코드", "품목명_고객표현", "점착제_고객표현", "재단구분", "기준폭", "담당부서", "영업담당부서", "담당자"]:
        if c in rec.columns:
...
with tab1:
    ...
    if q.empty:
        ...
        key_cols = [c for c in ["거래처", "품목코드", "점착제코드", "점착제명", "재단구분", "기준폭"] if c in q1.columns]
        ...
        ordered_cols = [
            "거래처", "품목코드", "점착제코드", "점착제명", "재단구분", "기준폭", "재단구분", "기준폭", "가로폭이력",
        ...
        clean_and_safe_display(
            ...
            text_cols=["거래처", "품목코드", "점착제코드", "점착제명", "재단구분", "기준폭", "재단구분", "기준폭", "가로폭이력", "최근날짜"],
        ...
        if len(all_months) < 2:
            ...
            if selected_customer != "선택하세요":
                ...
                if customer_data.empty:
                    ...
                    fig_total.update_layout(title="1️⃣ 업체 전체 월별 매출 추이")
                    st.plotly_chart(fig_total, use_container_width=True, key=f"tab4b_total_{selected_cust_name}")
                    ...
                    if not top_contrib.empty:
                        ...
                        st.plotly_chart(fig_contrib, use_container_width=True, key=f"tab4b_contrib_{selected_cust_name}")
                    ...
                    if not top_product_monthly.empty:
                        ...
                        st.plotly_chart(fig_products, use_container_width=True, key=f"tab4b_products_{selected_cust_name}")
                        st.caption("※ 각 포인트 값은 만원 단위입니다. 예: 4,500 = 4천5백만원")
                    ...
                    if not contribution_df.empty:
                        ...
                        st.plotly_chart(fig_bar, use_container_width=True, key=f"tab4b_compare_{selected_cust_name}")
                ...
                if not item_detail.empty:
                    show_cols = [
                        c for c in [
                            "품목표시", "총매출액", "총판매량",
                            "전반부_평균매출", "후반부_평균매출", "평균증감액",
                            "최근단가", "최근월매출", "변화율(%)", "재단구분", "기준폭", "가로폭이력", "최근일자"
                    ...
                    clean_and_safe_display(
                        show_df,
                        pinned_cols=["품목표시"],
                        text_cols=["품목표시", "재단구분", "기준폭", "가로폭이력", "최근일자"],
                        column_width_overrides={
                            "품목표시": 180,
                            "총매출액": 95,
                            "총판매량": 95,
                ...
                if not growth_customer_summary.empty:
                    ...
                    if not item_cust_growth.empty:
                        ...
                        if not top10_month.empty:
                            ...
                            st.plotly_chart(fig_top10, use_container_width=True, key=f"growth_top10_{selected_item}")
                        else:
...
with tab7:
    ...
    clean_and_safe_display(
        q[raw_cols],
        pinned_cols=["거래처", "품목코드"],
        text_cols=["거래처", "품목코드", "점착제코드", "점착제명", "재단구분", "기준폭", "재단구분", "기준폭", "가로폭이력", "최근날짜", "월", "비고", "재단구분", "기준폭", "담당부서", "영업담당부서", "담당자"],
        column_width_overrides={
            "날짜": 95,
            "거래처": 120,
            "품목코드": 145,
